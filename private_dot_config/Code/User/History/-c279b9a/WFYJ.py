...
class Fasta:
    ...
    def pick_taxids(self, taxids: list[str]) -> list[SeqRecord]:
        ...
        # Filter duplicates 
        def filter_duplicates(acc, current):
            first_member, _ = current
            if first_member not in acc['seen']:
                acc['seen'].add(first_member)
                acc['result'].append(current)
            return acc

        # Initial accumulator with a set to track unique first members and an empty list for the result
        initial_accumulator = {'seen': set(), 'result': []}

        # Use reduce to filter out duplicates
        filtered_tuples = reduce(filter_duplicates, tuple_iterator, initial_accumulator)
